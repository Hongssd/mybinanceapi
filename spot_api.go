...
const (
	...
	//子母账户接口
	SpotSubAccountList                     SpotApi = iota //查询子账户列表(适用主账户)
	SpotSubAccountUniversalTransferHistory                //查询子母账户万能划转历史查询
	SpotSubAccountAssets                                  //查询子账户资产 (适用主账户)(USER_DATA)
	SpotSubAccountFuturesAccount                          //查询子账户Futures账户详情V2 (适用主账户)
	SpotSubAccountApiIpRestriction                        //查询子账户API Key IP白名单 (适用母账户)
	SpotSubAccountTransferSubUserHistory                  //查询子账户划转历史 (仅适用子账户)
	SpotManagedSubAccountQueryTransLog                    //查询托管子账户的划转记录(适用交易团队子账户)(USER_DATA)
	SpotSubAccountVirtualSubAccount                       //创建虚拟子账户(适用主账户)
	SpotSubAccountUniversalTransfer                       //子母账户万能划转 (适用主账户)
	SpotSubAccountFuturesEnable                           //为子账户开通Futures (适用主账户)

	//杠杆账户接口
	SpotMarginAllPairs         //获取所有全仓杠杆交易对(MARKET_DATA)
	SpotMarginIsolatedAllPairs //获取所有逐仓杠杆交易对(USER_DATA)
	SpotMarginAccount          //查询全仓杠杆账户详情 (USER_DATA)
	SpotMarginIsolatedAccount  //查询逐仓杠杆账户详情 (USER_DATA)
	SpotMarginMaxBorrowable    //查询账户最大可借贷额度(USER_DATA)
	SpotMarginMaxTransferable  //查询最大可转出额 (USER_DATA)
	SpotMarginInterestHistory  //获取利息历史 (USER_DATA)
	SpotMarginOrderGet         //查询杠杆账户订单 (USER_DATA)
	SpotMarginOrderPost        //杠杆账户下单 (TRADE)
	SpotMarginOrderDelete      //撤销订单 (TRADE)
	SpotMarginAllOrders        //查询杠杆账户所有订单 (USER_DATA)
	SpotMarginOpenOrders       //查询杠杆账户挂单记录 (USER_DATA)
	SpotMarginTransfer         //全仓杠杆账户划转 (MARGIN)
	SpotMarginIsolatedTransfer //逐仓杠杆账户划转 (MARGIN)
	SpotMarginLoan             //杠杆账户借贷 (MARGIN) 支持逐仓和全仓
	SpotMarginRepay            //杠杆账户归还借贷 (MARGIN) 支持逐仓和全仓

	//现货账户接口
	SpotAccountApiTradingStatus //账户API交易状态(USER_DATA)
	SpotAccount                 //账户信息 (USER_DATA)
	SpotAssetTransferPost       //用户万向划转 (USER_DATA)
	SpotAssetTransferGet        //查询用户万向划转历史 (USER_DATA)
	SpotAssetTradeFee           //查询用户交易手续费率 (USER_DATA)

	//现货订单接口
	SpotOpenOrders  //查询当前挂单 (USER_DATA)
	SpotAllOrders   //查询所有订单 (USER_DATA)
	SpotOrderGet    //查询订单 (USER_DATA)
	SpotOrderPost   //下单 (TRADE)
